...
export type RuleFn = (file: ScannedFile, config: ArchDriftConfig, projectRoot: string) => Violation[]
...
export function runRules(files: ScannedFile[], config: ArchDriftConfig, projectRoot: string): Violation[] {
  ...
  for (const file of files) {
    for (const [ruleName, ruleFn] of Object.entries(ruleRegistry)) {
      ...
      if (ruleConfig && ruleConfig !== 'off') {
        violations.push(...ruleFn(file, config, projectRoot))
      }
    }
  }
